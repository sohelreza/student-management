...
class ZoomClass extends Controller
{
    ...
    public function add(){
    	...
    	return view('admin.liveClass.addClass',compact('branches','teachers','zoomapis'));
    }
    ...
    public function edit($id){
        ...
        $teachers=Admin::where('role_id',4)->get();  
        $zoomapis=ZoomApi::all();                  
    	return view('admin.liveClass.editClass',compact('branches','teachers','zoomapis','meeting'));
    }
    ...
    public function update($id, Request $request)
    {
	    ...
	    $meeting=ZoomMetting::find($request->id);
        $path = 'meetings/'.$meeting->id;
     //    $response = $this->zoomPatch($path,$request->zoom_api, [
	    //     'topic' => $request->topic,
	    //     'description' => $request->description,
	    //     'type' => self::MEETING_TYPE_SCHEDULE,
	    //     'start_time' => $this->toZoomTimeFormat($request->date.' '.$request->time),
    ...
    public function list()
	{
	    ...
	    return view('admin.liveClass.classList',compact('data'));
	}
	...
	public function class_live_meetings(Request $request)
	{
	    ...
	    $student_class=DB::table('student_subjects')
	                    ->select('student_subjects.*','subjects.name','zoom_mettings.join_url','zoom_mettings.topic' ,'zoom_mettings.duration','zoom_mettings.when','zoom_mettings.live_status')
	                    ...
	                    ->leftJoin('zoom_mettings', function($join) use($student_branch,$student_class,$student_batch)
                         {
                             ...
                             ->whereIn('zoom_mettings.live_status',[1,2])
	...
	public function submit_attendance(Request $request)
	{
		...
		// $zoom_meeting=ZoomMetting::find($request->meeting_id[$key]);
	 //    $students=User::where('batch_id',$zoom_meeting->batch)
	 //                 ->where('class_id',$zoom_meeting->class)
	 //                 ->where('branch_id',$zoom_meeting->branch)
	 //                 ->get();
	...
	public function class_live_dashboard(Request $request)
	{
	    $student=User::find($request->student_id);
