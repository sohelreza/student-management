   ...
   <header class="page-header">
					...
					<section class="panel">
							<header class="panel-heading">
								...
								<h2 class="panel-title">Edit Branches</h2>
							</header>
							<div class="panel-body">
								<form class="" action="{{ action('BranchController@update',$branch->id)}}" method="post">
									@csrf
									<input type="hidden" name="_method" value="PUT">
									<div class="form-group">
										<label class="col-md-2 control-label" for="inputDefault">Name</label>
											<div class="col-md-6">
												<input type="text" class="form-control" name="name" id="inputDefault" value="{{$branch->name}}">
											</div>
									</div>
									<div class="form-group">
										<label class="col-md-2 control-label" for="textareaAutosize">Address</label>
										    <div class="col-md-6">
												<textarea class="form-control" name="address" rows="3" id="textareaAutosize" data-plugin-textarea-autosize value="">{{$branch->address}}</textarea>
											</div>
									</div>
									<div class="form-group">
										...
										<div class="col-md-6">
											<select class="form-control mb-md" name="student_type" id="student_type">
												<option value="0" {{ $branch->student_type == 0 ? 'selected' : '' }}>Offline</option>
												<option value="1" {{ $branch->student_type == 1 ? 'selected' : '' }}>Online</option>
												<option value="2" {{ $branch->student_type == 2 ? 'selected' : '' }}>Both</option>
