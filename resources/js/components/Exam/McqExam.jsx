...
class McqExam extends Component {
        ...
        ExamQus.getQuestion(data).then(res => {
            if (res.data.exam_enroll == null || res.data.exam_enroll == 0) {
                this.setState({
                    ...
                    start_time: res.data.exam.start_time
                    // answer: res.data.exam_questions[currentIndex].options
                });
                // this.startTimer();
                if (this.state.start_time <= this.state.currentTime) {
                    console.log("log1" + this.state.currentTime);
                    console.log("log2" + this.state.end_time);
                    ...
                    if (endTime[0] == currentTime[0]) {
                        ...
                        console.log("before1" + this.state.currentTime);
                    }
                } else {
                    console.log("before1" + this.state.currentTime);
    ...
    nextQuestionHandler() {
        ...
        if (this.state.end_time > this.state.currentTime) {
            ...
            if (this.state.userAnswer.length == 0) {
                this.setState({
                    notAnswers: this.state.notAnswers.concat(
                        this.state.currentIndex
                    )
                });
            } else {
            ...
            if (this.state.notAnswerFlag == 0) {
                this.setState({
                    currentIndex: this.state.currentIndex + 1
                    // seconds: this.state.exam_durations
                });
            } else {
                this.state.notAnswers.map((res, i) => {
                    console.log(res)
                    console.log(i)
                    this.setState({
                        currentIndex: res
    ...
    previousQuestionHandler() {
        // clearInterval(this.timer);
        if (this.state.end_time > this.state.currentTime) {
            console.log(this.state.notAnswers);
            this.setState({
                currentIndex: this.state.notAnswers[0],
                notAnswerFlag: 1
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.state.currentIndex !== prevState.currentIndex) {
            console.log("helloa" + this.state.currentIndex);
            // ExamQus.getQuestion(data).then(res => {
            // this.setState({
            //     disabled: true,
            //     exam_id: res.data.exam.id,
            ...
            if (endTime[0] == currentTime[0]) {
                ...
                console.log("before2" + this.state.currentTime);
            }
            // });
        }
    }
    ...
    nextQuestion(currentIndex) {
        if (currentIndex < this.state.totalQuestion - 1) {
            ...
            if (this.state.notAnswers.length !== 0) {
                return (
                    <div className="text-center">
                        <button
                            className="btn btn-success update-button"
                            onClick={this.previousQuestionHandler.bind(this)}
                        >
                            Previous {"<"}
                        </button>
                    </div>
